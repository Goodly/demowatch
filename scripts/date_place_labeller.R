...
### testing on metadata_with_tua
testFunc <- function(article_data) {
  pdate_weekday_number <- weekdayNumber(article_data$date_published)
  event_weekday_number <- t(data.frame(lapply(article_data$article_text, return_days)))
  # for each item in this event_weekday_number:
  #   if item is NA --> go to second hierarchical step. call return_days on article_text
  
  # CHANGE IT TO CALL ON TUA FIRST
  phase1
  phase2
  phase3
  phase4
